...
--
-- Alle Seiten werden DEAKTIVIERT angelegt (is_active = 0). Sie sind
-- damit im CMS vorhanden und bearbeitbar, aber weder fuer Besucher
-- noch fuer Google sichtbar. Grund: die Texte enthalten Stellen
-- [ANGABE FEHLT: ...], die nur der Kunde beantworten kann.
--
-- Freischalten im CMS unter Seiten, sobald die Angaben da sind.
--
...
INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'text-block', '{"heading":"Sanitär Vorwandelemente: Aufbau, Montage und Beplankung erklärt","body":"<p>Sanitär Vorwandelemente tragen WC, Waschtisch, Dusche oder Urinal und nehmen Zu- und Ablaufleitungen auf, ohne dass jemand die Rohre sieht. Sie bestehen aus einem verzinkten Stahlrahmen, der am Boden und an der Rohbauwand verschraubt wird, und werden anschliessend beplankt und gespachtelt. Auf dieser Seite lesen Sie, wie ein solches Element aufgebaut ist, welche Masse üblich sind, warum die Beplankung im Nassbereich eine eigene Rolle spielt und wo im Bauablauf die Schallentkopplung entschieden wird. SUI Innova aus Pfäffikon fertigt GIS-Elemente in der eigenen Werkstatt fix und fertig verrohrt vor, montiert sie auf der Baustelle und übernimmt Beplankung, Spachtelung und Ausflockung. Sie erhalten die Wand also nicht in Teilen, sondern als eine Leistung.</p>","alignment":"left"}', 20, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 20);

INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'text-block', '{"heading":"","body":"<h2>Ein Vorwandelement ist Tragwerk und Installationsraum zugleich</h2><p>Das Element nimmt die Lasten von WC, Waschtisch oder Stützgriffen auf und leitet sie in Boden und Rohbauwand ab. Gleichzeitig läuft im Innenraum alles, was nicht sichtbar sein soll: Kalt- und Warmwasser, Abwasser, teilweise Elektro. Deshalb wird die Position jeder Leitung festgelegt, bevor die erste Platte montiert wird. Fehler in dieser Phase kosten später Aufbruch und Zeit.</p><h2>Vorfabrikation verlagert die knifflige Arbeit von der Baustelle in die Werkstatt</h2><p>In der Werkstatt lassen sich Elemente auf dem Tisch verrohren, ausrichten und prüfen. Auf der Baustelle wird das fertige Element nur noch gestellt, befestigt und angeschlossen. Das verkürzt die Zeit, in der andere Gewerke warten müssen, und reduziert Anpassungen vor Ort. SUI Innova liefert GIS-Elemente fix und fertig verrohrt aus der eigenen Werkstatt an.</p><h2>Masse und Höhen ergeben sich aus dem Apparat, nicht aus dem Gefühl</h2><p>Übliche Bauhöhen sind das halbhohe Element für die freistehende Vorwand und das raumhohe Element bis zur Decke. Die Fertighöhe der WC-Keramik und die Lage der Spülkasten-Betätigung richten sich nach Herstellerangaben und nach der geplanten Bodenaufbauhöhe. Wird der Bodenaufbau später geändert, stimmt die Höhe nicht mehr. Klären Sie die Aufbauhöhe deshalb vorgängig mit Architektur und Plattenleger. [ANGABE FEHLT: Standardhöhen und Systeme, die SUI Innova verarbeitet]</p><h2>Im Nassbereich entscheidet die Beplankung über die Lebensdauer</h2><p>Hinter Dusche und Badewanne gehört eine Platte, die Feuchtigkeit verträgt. AquaPanel wird in Feuchträumen und Nasszellen eingesetzt und bildet den Untergrund für Abdichtung und Plättli. Die Fugen und Anschlüsse werden gespachtelt, damit die Fläche ohne Absatz weitergeht. SUI Innova übernimmt Beplankung und Spachtelung im gleichen Auftrag wie die Montage.</p><h2>Schall wird an der Befestigung entschieden, nicht am Rohr</h2><p>Spülgeräusche wandern über Körperschall in angrenzende Räume, wenn Element und Rohre starr mit der Rohbauwand verbunden sind. Entkoppelte Befestigungen, Schallschutzsets und eine Ausflockung des Hohlraums dämpfen diesen Weg. Besonders wichtig ist das bei Wänden zu Schlafräumen und in Mehrfamilienhäusern. [ANGABE FEHLT: gewünschte Angaben zu SilentPanel und erreichbaren Werten]</p><h2>Von der Planung bis zur spachtelfertigen Wand aus einer Hand</h2><p>Sie schicken uns Pläne oder Schemas, wir klären die Apparatepositionen und fertigen die Elemente vor. Danach montieren wir auf der Baustelle, beplanken und spachteln. Sie haben eine Ansprechperson für Vorfabrikation, Montage und Beplankung statt drei Schnittstellen. Was das für Ihr Projekt konkret heisst, klären wir am besten anhand Ihrer Unterlagen.</p>","alignment":"left"}', 30, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 30);

INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'faq', '{"heading":"Fragen und Antworten","subtitle":"","items":[{"question":"Wie tief ist eine Sanitärvorwand üblicherweise?","answer":"<p>Die Tiefe richtet sich nach dem Spülkasten und den Leitungen dahinter. Für ein WC-Element rechnet man mit rund 15 bis 20 Zentimetern Rahmentiefe plus Beplankung. Bei zusätzlichen Steigleitungen oder Lüftungen wird die Vorwand entsprechend tiefer.</p>"},{"question":"Kann ich später etwas an der Wand befestigen?","answer":"<p>Nur dort, wo Traversen oder Verstärkungen eingebaut sind. Spiegelschrank, Handtuchhalter oder Stützgriffe sollten Sie deshalb vor der Beplankung anmelden. Nachträglich befestigt man in einer beplankten Vorwand nur leichte Lasten.</p>"},{"question":"Wer koordiniert Sanitär, Trockenbau und Plattenleger?","answer":"<p>Bei getrennten Aufträgen liegt die Koordination bei der Bauleitung. Wenn Vorfabrikation, Montage und Beplankung in einer Hand liegen, entfällt ein Teil dieser Abstimmung. SUI Innova übergibt die Wand spachtelfertig an das Folgegewerk.</p>"}]}', 40, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 40);

INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'cta-banner', '{"heading":"Pläne einsenden, Offerte erhalten","body":"Senden Sie uns Ihre Pläne, wir melden uns mit Rückfragen zu Apparatepositionen und einer Offerte für Vorfabrikation, Montage und Beplankung.","button_text":"Pläne einsenden","button_url":"/kontakt"}', 50, 1
FROM DUAL
WHERE @pid IS NOT NULL
...
INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'text-block', '{"heading":"Sanitär GIS Elemente bestellen bei SUI Innova","body":"<p>Sie wollen Sanitär GIS Elemente bestellen, die auf der Baustelle nur noch gestellt und angeschlossen werden müssen. Genau das liefern wir. In unserer Werkstatt in Pfäffikon bauen wir GIS Elemente nach Ihren Plänen auf, verrohren sie fix und fertig und bereiten sie für den Transport vor. Auf Wunsch übernehmen wir auch Montage, Beplankung mit AquaPanel, Spachtelung und Ausflockung mit SilentPanel. Sie erhalten alles aus einer Hand und haben eine Ansprechperson für den ganzen Ablauf. Schicken Sie uns Ihre Sanitärpläne oder eine Skizze mit Apparateliste. Wir prüfen die Unterlagen, melden uns bei offenen Punkten und erstellen Ihnen eine Offerte mit Preis und möglichem Liefertermin.</p>","alignment":"left"}', 20, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 20);

INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'text-block', '{"heading":"","body":"<h2>So läuft die Bestellung ab</h2><p>Sie senden uns Ihre Sanitärpläne, die Apparateliste und den gewünschten Liefertermin. Wir prüfen die Unterlagen und klären offene Punkte direkt mit Ihnen oder mit dem Planer. Danach erhalten Sie eine Offerte mit Positionen, Preis und Terminvorschlag. Nach Ihrer Freigabe starten wir die Vorfabrikation in der Werkstatt.</p><h2>Fix und fertig verrohrt aus der Werkstatt</h2><p>Wir bauen die GIS Elemente auf und verrohren sie komplett, bevor sie die Werkstatt verlassen. Dadurch verschieben Sie Arbeitsstunden von der Baustelle in eine Umgebung mit festen Arbeitsplätzen und Material zur Hand. Auf der Baustelle bleibt das Stellen, Fixieren und Anschliessen. Das verkürzt die Zeit, in der andere Gewerke warten müssen.</p><h2>Was Sie zusätzlich zum Element bestellen können</h2><p>Viele Kunden bestellen nicht nur das nackte Element. Wir montieren die Elemente direkt auf Ihrer Baustelle, beplanken sie mit AquaPanel für Feuchträume und Nasszellen und spachteln die Flächen. Für den Schallschutz flocken wir die Vorwand mit SilentPanel aus. Sie entscheiden, wo unsere Arbeit endet und Ihre beginnt.</p><h2>Angaben, die wir für eine Offerte brauchen</h2><p>Für eine belastbare Offerte brauchen wir den Sanitärplan oder eine bemasste Skizze, die Anzahl und Art der Apparate und die Höhen der Vorwand. Nützlich sind zudem Angaben zum Wandaufbau, zur gewünschten Beplankung und zu Anforderungen an den Schallschutz. Nennen Sie uns auch die Zufahrt und die Etage, das beeinflusst Anlieferung und Handling. Fehlt etwas, fragen wir nach, bevor wir rechnen.</p><h2>Für Sanitärunternehmen, Generalunternehmer und Bauherren</h2><p>Wir arbeiten für Sanitärbetriebe, die Kapazität in der Werkstatt brauchen, und für Bauleitungen, die einen fixen Liefertermin wollen. Kleine Umbauten mit einem einzelnen Element sind ebenso möglich wie Serien für ganze Geschosse. Sagen Sie uns, wie viele Elemente Sie brauchen und bis wann. Wir sagen Ihnen ehrlich, ob wir den Termin halten können.</p><h2>Lieferung und Termine</h2><p>Die Elemente werden montagefertig angeliefert und nach Absprache abgeladen. Den Liefertermin halten wir in der Offerte fest, damit Ihre Bauleitung damit planen kann. Verschiebt sich Ihr Bauprogramm, melden Sie sich frühzeitig, dann suchen wir einen neuen Termin. [ANGABE FEHLT: übliche Lieferfrist in Wochen und Liefergebiet]</p>","alignment":"left"}', 30, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 30);

INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'faq', '{"heading":"Fragen und Antworten","subtitle":"","items":[{"question":"Kann ich einzelne GIS Elemente bestellen oder nur ganze Serien?","answer":"<p>Beides ist möglich. Wir fertigen einzelne Elemente für Umbauten ebenso wie Serien für Neubauten. Nennen Sie uns die Stückzahl bei der Anfrage.</p>"},{"question":"Muss ich das Material selber liefern?","answer":"<p>Das klären wir vor der Offerte. Sie können Material beistellen oder wir beschaffen es. [ANGABE FEHLT: ob SUI Innova Material selber beschafft und liefert]</p>"},{"question":"Übernehmen Sie auch die Montage auf der Baustelle?","answer":"<p>Ja, wir montieren die Elemente direkt auf Ihrer Baustelle. Auf Wunsch beplanken und spachteln wir die Wand anschliessend. Sie bestellen also nur die Vorfabrikation oder den ganzen Ablauf bis zur fertigen Wand.</p>"}]}', 40, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 40);

INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'cta-banner', '{"heading":"GIS Elemente anfragen","body":"Senden Sie uns Ihre Sanitärpläne über das Kontaktformular, wir melden uns mit Rückfragen und einer Offerte samt Terminvorschlag.","button_text":"Pläne einsenden","button_url":"/kontakt"}', 50, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 50);
...
INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'text-block', '{"heading":"GIS Elemente beplanken: von der Vorfabrikation bis zur fertigen Wand","body":"<p>Wir beplanken GIS Elemente, in unserer Werkstatt in Pfäffikon und direkt auf Ihrer Baustelle. Sie erhalten die Elemente verrohrt, beplankt und gespachtelt, bereit für Plättli, Farbe oder SilentPanel. In Nasszellen setzen wir AquaPanel ein, in trockenen Bereichen Gipsplatten nach Ihren Vorgaben. [ANGABE FEHLT: verwendete Plattentypen, Stärken und Hersteller] Wir arbeiten mit Sanitärfirmen, Generalunternehmen und Bauleitungen zusammen und richten uns nach Ihrem Bauprogramm. Nennen Sie uns Stückzahl, Wandaufbau und Termin, dann rechnen wir Ihnen eine Offerte. Vorfabrikation, Montage und Beplankung kommen bei uns aus einer Hand. Das spart Schnittstellen, Rückfragen und Wartezeiten auf der Baustelle. [ANGABE FEHLT: Einzugsgebiet, in dem SUI Innova arbeitet]</p>","alignment":"left"}', 20, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 20);

INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'text-block', '{"heading":"","body":"<h2>Beplankung in der Werkstatt spart Zeit auf der Baustelle</h2><p>Wir verrohren und beplanken die GIS Elemente vorgängig in unserer Werkstatt. Auf der Baustelle wird das Element nur noch versetzt und angeschlossen. So verkürzt sich die Zeit, in der andere Handwerker warten müssen. Transport und Anlieferung stimmen wir auf Ihren Bauablauf ab. [ANGABE FEHLT: maximale Elementgrösse und Transportmöglichkeiten]</p><h2>AquaPanel für Nasszellen, Gipsplatten für trockene Räume</h2><p>In Duschen, Bädern und Nasszellen beplanken wir mit AquaPanel, weil die Platte Feuchtigkeit verträgt. In trockenen Bereichen arbeiten wir mit Gipsplatten. Welche Platte wo zum Einsatz kommt, halten wir vor Baubeginn schriftlich fest. Sagen Sie uns, ob Sie Plättli, Verputz oder eine Beschichtung planen, danach richtet sich der Aufbau.</p><h2>Spachteln bis zur gewünschten Qualitätsstufe</h2><p>Nach der Beplankung verspachteln wir Fugen, Kanten und Schraubenköpfe. Sie geben die Qualitätsstufe vor, wir liefern die Fläche entsprechend ab. Damit übernimmt der Maler oder Plattenleger eine Wand, an der er direkt weiterarbeiten kann. [ANGABE FEHLT: angebotene Qualitätsstufen nach SIA oder Q1 bis Q4]</p><h2>Ausflockung und SilentPanel gegen Schall</h2><p>Auf Wunsch flocken wir die Elemente aus und montieren SilentPanel. Beides reduziert die Schallübertragung von Leitungen in angrenzende Räume. Sinnvoll ist das vor allem bei Wohnungstrennwänden und bei Bädern neben Schlafzimmern. Klären Sie die Anforderung frühzeitig mit uns, denn sie beeinflusst den Wandaufbau. [ANGABE FEHLT: erreichbare Schallschutzwerte]</p><h2>Ein Ansprechpartner für Vorfabrikation, Montage und Beplankung</h2><p>Sie beauftragen eine Firma statt drei. Wir montieren die Elemente selbst und beplanken sie anschliessend, deshalb gibt es keine Diskussion über Vorleistungen. Bei Änderungen auf der Baustelle passen wir die Elemente an. Den Stand melden wir Ihnen laufend, damit Ihre Terminplanung hält.</p><h2>So läuft ein Auftrag ab</h2><p>Sie senden uns Pläne, Stückzahlen und den gewünschten Termin. Wir prüfen die Unterlagen und stellen Ihnen eine Offerte mit Positionen und Preisen zu. Nach der Freigabe fixieren wir die Produktions- und Montagetermine. Zum Schluss übergeben wir die beplankten Wände und melden Ihnen, was noch offen ist.</p>","alignment":"left"}', 30, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 30);

INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'faq', '{"heading":"Fragen und Antworten","subtitle":"","items":[{"question":"Beplanken Sie auch GIS Elemente, die wir selbst montiert haben?","answer":"<p>Ja, wir übernehmen die Beplankung auch dann, wenn Vorfabrikation und Montage bei Ihnen liegen. Wir prüfen vor dem Start, ob die Elemente ausgerichtet und die Leitungen abgedrückt sind. Mängel melden wir Ihnen schriftlich, bevor wir die Platten setzen.</p>"},{"question":"Welche Platten verwenden Sie in Nasszellen?","answer":"<p>In Nasszellen und Feuchträumen beplanken wir mit AquaPanel. Für trockene Bereiche kommen Gipsplatten zum Einsatz. Weichen Ihre Vorgaben davon ab, halten wir uns an Ihr Devis.</p>"},{"question":"Wie schnell können Sie mit der Beplankung beginnen?","answer":"<p>Das hängt von der aktuellen Auslastung und der Stückzahl ab. Melden Sie sich mit Ihrem Bauprogramm, dann nennen wir Ihnen einen verbindlichen Termin. [ANGABE FEHLT: übliche Vorlaufzeit in Wochen]</p>"}]}', 40, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 40);

INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'cta-banner', '{"heading":"Offerte für die Beplankung anfragen","body":"Senden Sie uns Pläne, Stückzahl und Wunschtermin, Sie erhalten von uns eine Offerte mit Positionen und Preisen.","button_text":"Pläne einsenden","button_url":"/kontakt"}', 50, 1
FROM DUAL
WHERE @pid IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM sections WHERE page_id = @pid AND sort_order = 50);
