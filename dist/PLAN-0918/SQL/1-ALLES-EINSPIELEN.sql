...
-- Die drei Sektionen, die dieses Skript selbst anlegt, fallen
-- vorher weg und werden gleich neu geschrieben. Sonst bliebe
-- beim zweiten Durchlauf die aeltere Textfassung stehen: die
-- Seite hat ihr eigenes Layout, sie wird nicht komplett neu
-- aufgebaut wie die anderen. Leistungskarten, Kopfbilder und
-- Handlungsaufforderung bleiben unberuehrt.
DELETE FROM sections
WHERE page_id = @pid AND @pid IS NOT NULL
  AND type IN ('content-grid', 'faq', 'link-list');
...
INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'link-list', '{"heading":"Passend dazu","items":[{"slug":"gis-elemente-beplanken","text":"GIS Elemente montieren lassen"},{"slug":"sanitaerelemente-vorfabrizieren","text":"Sanitärelemente vorfabrizieren lassen"}]}', 40, 1
FROM DUAL WHERE @pid IS NOT NULL;
...
INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'link-list', '{"heading":"Passend dazu","items":[{"slug":"leistungen","text":"Sanitär Vorfabrikation: alle Leistungen"},{"slug":"sanitaer-gis-elemente-bestellen","text":"GIS Elemente verrohrt bestellen"},{"slug":"sanitaerelemente-vorfabrizieren","text":"Sanitärelemente vorfabrizieren lassen"},{"slug":"sanitaerelemente-montieren","text":"Sanitärelemente montieren"},{"slug":"sanitaer-vorwaende","text":"Sanitär Vorwände vorfabriziert und montiert"},{"slug":"sanitaer-vorwandelemente-beplanken","text":"Sanitär Vorwandelemente beplanken"}]}', 40, 1
FROM DUAL WHERE @pid IS NOT NULL;
...
INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'link-list', '{"heading":"Passend dazu","items":[{"slug":"leistungen","text":"Sanitär Vorfabrikation: alle Leistungen"},{"slug":"sanitaer-gis-elemente-bestellen","text":"Sanitär GIS Elemente bestellen"},{"slug":"gis-elemente-beplanken","text":"GIS Elemente beplanken und montieren"},{"slug":"sanitaerelemente-montieren","text":"Montage von Sanitär Vorwandelementen"},{"slug":"sanitaer-vorwandelemente-bestellen","text":"Sanitär Vorwandelemente bestellen"},{"slug":"sanitaer-vorwaende","text":"Sanitär Vorwände vorfabriziert und montiert"},{"slug":"sanitaer-vorwandelemente-beplanken","text":"Sanitär Vorwandelemente beplanken"}]}', 40, 1
FROM DUAL WHERE @pid IS NOT NULL;
...
INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'link-list', '{"heading":"Passend dazu","items":[{"slug":"sanitaer-gis-elemente-bestellen","text":"GIS Elemente bestellen"},{"slug":"sanitaer-vorwandelemente","text":"Sanitär Vorwandelemente: Aufbau und Montage"},{"slug":"sanitaer-vorwandelemente-beplanken","text":"Sanitär Vorwandelemente beplanken"}]}', 40, 1
FROM DUAL WHERE @pid IS NOT NULL;
...
INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'link-list', '{"heading":"Passend dazu","items":[{"slug":"sanitaer-vorwandelemente","text":"Sanitär Vorwandelemente: Aufbau und Montage"},{"slug":"sanitaer-vorwandelemente-bestellen","text":"Sanitär Vorwandelemente bestellen"},{"slug":"gis-elemente-beplanken","text":"Beplankung von GIS Elementen"}]}', 40, 1
FROM DUAL WHERE @pid IS NOT NULL;
...
INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'link-list', '{"heading":"Passend dazu","items":[{"slug":"leistungen","text":"Sanitär Vorfabrikation: alle Leistungen"},{"slug":"sanitaer-gis-elemente-bestellen","text":"vorfabrizierte GIS Elemente bestellen"},{"slug":"gis-elemente-beplanken","text":"beplankte GIS Elemente aus unserer Werkstatt"}]}', @sort, 1
FROM DUAL WHERE @pid IS NOT NULL;
...
-- ════════════════════════════════════════════════════════════
-- Startseite — Linkblock auf die neun Leistungsseiten
-- ════════════════════════════════════════════════════════════
SET @pid = (SELECT id FROM pages WHERE is_homepage = 1 LIMIT 1);

DELETE FROM sections
WHERE page_id = @pid AND @pid IS NOT NULL AND type = 'link-list';

-- Sortierung auf Zehnerschritte, damit dazwischen Platz bleibt.
SET @r = 0;
UPDATE sections SET sort_order = (@r := @r + 10)
WHERE page_id = @pid AND @pid IS NOT NULL ORDER BY sort_order, id;

-- Der Block gehoert hinter den Inhalt und vor die letzte
-- Handlungsaufforderung. Gibt es keine, kommt er ganz unten hin.
SET @cta = (SELECT MAX(sort_order) FROM sections
            WHERE page_id = @pid AND type = 'cta-banner');
SET @sort = COALESCE(@cta - 5,
            (SELECT COALESCE(MAX(sort_order), 0) + 10 FROM sections WHERE page_id = @pid));

INSERT INTO sections (page_id, type, content, sort_order, is_active)
SELECT @pid, 'link-list', '{"heading":"Unsere Leistungen im Detail","items":[{"slug":"leistungen","text":"Sanitär Vorfabrikation: alle Leistungen"},{"slug":"sanitaer-gis-elemente-bestellen","text":"Sanitär GIS Elemente bestellen"},{"slug":"gis-elemente-beplanken","text":"GIS Elemente beplanken und montieren"},{"slug":"sanitaer-vorwandelemente","text":"Sanitär Vorwandelemente: Aufbau und Montage"},{"slug":"sanitaer-vorwandelemente-bestellen","text":"Sanitär Vorwandelemente bestellen"},{"slug":"sanitaer-vorwaende","text":"Sanitär Vorwände vorfabriziert und montiert"},{"slug":"sanitaer-vorwandelemente-beplanken","text":"Sanitär Vorwandelemente beplanken"},{"slug":"sanitaerelemente-vorfabrizieren","text":"Sanitärelemente vorfabrizieren lassen"},{"slug":"sanitaerelemente-montieren","text":"Sanitärelemente montieren"}]}', @sort, 1
FROM DUAL WHERE @pid IS NOT NULL;
...
-- ════════════════════════════════════════════════════════════
-- Nachtrag: Cookie-Banner ohne Gedankenstrich
--
-- Nur, wenn der Text noch eine der bekannten alten Fassungen
-- ist. Ein selbst geschriebener Text bleibt unangetastet.
-- ════════════════════════════════════════════════════════════

UPDATE settings
SET setting_val = 'Wir verwenden technisch notwendige Cookies. Mit Ihrer Einwilligung kommen Statistik-Cookies von Google Analytics dazu, für die anonyme Auswertung der Nutzung.'
WHERE setting_key = 'cookie_text'
  AND (setting_val LIKE '%Einwilligung – Statistik-Cookies%'
    OR setting_val LIKE '%Einwilligung — Statistik-Cookies%'
    OR setting_val LIKE '%und, mit Ihrer Einwilligung, Statistik-Cookies%'
    OR setting_val LIKE '%zur anonymen Analyse der Website-Nutzung%');
